...
export function HomeWorkspace() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 pt-10 sm:px-6">
      ...
      {!user && (
        <div className="mb-5 flex flex-wrap items-center gap-3 rounded-xl border border-violet-400/20 bg-violet-500/8 p-3.5">
          <Sparkles className="size-4.5 shrink-0 text-violet-300" />
          <p className="min-w-0 flex-1 text-[0.82rem] leading-relaxed text-violet-100/85">
      ...
      <div className="grid gap-5 lg:grid-cols-[1.12fr_0.88fr]">
        <GlassCard strong className="p-5 sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs tracking-[0.16em] text-slate-500 uppercase">Đang làm dở</p>
          ...
          {resume.length === 0 ? (
            <div className="mt-6 rounded-xl border border-dashed border-white/12 bg-white/3 p-5">
              <p className="text-sm text-slate-300">Chưa có bài làm dở nào.</p>
        ...
        <GlassCard className="flex items-center gap-5 p-5 sm:p-6">
          <div className="shrink-0">
            {latestOverall !== null ? (
              ...
              <div className="grid size-[132px] place-items-center rounded-full border border-dashed border-white/15 text-center">
                <span className="text-xs leading-relaxed text-slate-500">Chưa có<br />điểm</span>
              </div>
            )}
          </div>
          <div className="min-w-0">
            <p className="text-xs tracking-[0.16em] text-slate-500 uppercase">Tổng quan</p>
            <h3 className="font-display mt-1 text-xl font-semibold text-white">
              ...
              <GlassCard hover className="h-full p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[0.68rem] text-slate-500">{skill.count} lượt làm</p>
                  </div>
                  <span
                    className="ml-auto font-display text-xl font-semibold tabular-nums"
                    style={{ color: skill.latest === null ? "#64748b" : skill.color }}
                  >
                    {skill.latest === null ? "--" : skill.latest.toFixed(1)}
                  </span>
                </div>
                <div className="mt-4">
                  <div className="mb-1.5 flex justify-between text-[0.68rem] text-slate-500">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
        <GlassCard className="p-5 sm:p-6">
          <div className="flex flex-wrap items-center gap-3">
            <div className="grid size-10 place-items-center rounded-xl bg-violet-500/15">
              <BrainCircuit className="size-5 text-violet-300" />
            </div>
            <div>
              <p className="text-xs tracking-[0.16em] text-slate-500 uppercase">AI coach</p>
          ...
          {analysis ? (
            <div className="mt-5 space-y-3">
              <div className="rounded-xl border border-violet-400/20 bg-violet-500/8 p-4">
                <p className="text-sm leading-relaxed text-violet-100">{analysis.summary}</p>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl border border-white/8 bg-white/3 p-3.5">
                  <p className="text-xs text-slate-500">Nên ưu tiên</p>
                  <p className="mt-1 font-medium text-white">
                    {analysis.prioritySkill === "full"
                      ? "Đề IELTS đầy đủ"
                      : SKILL_MAP[analysis.prioritySkill].name}
                  </p>
                  <p className="mt-1 text-xs leading-relaxed text-slate-400">{analysis.reason}</p>
                </div>
                <div className="rounded-xl border border-white/8 bg-white/3 p-3.5">
                  <p className="text-xs text-slate-500">Bài nên làm</p>
                  <p className="mt-1 font-medium text-white">{analysis.recommendation}</p>
            ...
            <div className="mt-5 rounded-xl border border-dashed border-white/12 bg-white/3 p-4">
          ...
          {analysisError && analysis && <p className="mt-3 text-xs text-rose-300">{analysisError}</p>}
        </GlassCard>

        <GlassCard className="p-5 sm:p-6">
          <div className="flex items-center gap-3">
            <div className="grid size-10 place-items-center rounded-xl bg-emerald-500/14">
              <CheckCircle2 className="size-5 text-emerald-300" />
            </div>
            <div>
              <p className="text-xs tracking-[0.16em] text-slate-500 uppercase">Full test</p>
              <h3 className="font-display mt-1 text-lg font-semibold text-white">
          ...
          {latestFull ? (
            <div className="mt-5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-medium text-white">{latestFull.testTitle}</p>
                  <p className="mt-1 text-xs text-slate-500">{formatDate(latestFull.createdAt)}</p>
                </div>
                <span
              ...
              <div className="mt-4 grid grid-cols-4 gap-2">
                {SKILLS.map((skill) => (
                  <div key={skill.key} className="rounded-lg border border-white/8 bg-white/3 p-2 text-center">
                    <p className="text-[0.62rem] text-slate-500">{skill.name}</p>
...
function ResumeRow({ item }: { item: ResumeProgress }) {
  ...
  return (
    <Link
      href={item.href}
      className="group block rounded-xl border border-white/8 bg-white/3 p-3.5 transition-colors hover:border-white/18 hover:bg-white/6"
    >
      <div className="flex items-center gap-3">
        <span className="grid size-9 shrink-0 place-items-center rounded-lg" style={{ background: `${color}1c` }}>
        ...
        <ChevronRight className="size-4 shrink-0 text-slate-500 transition-transform group-hover:translate-x-0.5 group-hover:text-white" />
      </div>
      <div className="mt-3 flex items-center gap-2">
        <ProgressBar value={item.completed} max={item.total} color={color} height={4} />
        <span className="w-9 text-right text-[0.68rem] tabular-nums text-slate-500">{percent}%</span>
      </div>
    </Link>
  );
}
